...
class Cortex(Dispatcher):
    ...
    def sub_request_GRH(self, stream, threshold, delay, key):
        ...
        if result_dic.get('com') != None:
            ...
            self.emit('new_com_data', data=com_data)
            if com_data['action'] == 'push' and 100*float(com_data['power']) >= int(threshold) and (float(com_data['time']) - temp_time) >= delay:
                if key[0] == '':
            ...
            elif com_data['action'] == 'pull' and 100*float(com_data['power']) >= int(threshold) and (float(com_data['time']) - temp_time) >= delay:
                if key[1] == '':
                    print("please assign a corresponding keyboard key to this command")
                elif key[1] == 'space':
                    press(Key.space)
            ...
            elif com_data['action'] == 'lift' and 100*float(com_data['power']) >= int(threshold) and (float(com_data['time']) - temp_time) >= delay:
                if key[2] == '':
                    print("please assign a corresponding keyboard key to this command")
                elif key[2] == 'space':
                    press(Key.space)
                ...
                config["time"] = float(com_data['time'])
                json.dump(config, open("time_config.json", "w"), indent=4, sort_keys=True)
            elif com_data['action'] == 'drop' and 100*float(com_data['power']) >= int(threshold) and (float(com_data['time']) - temp_time) >= delay:
                if key[3] == '':
                    print("please assign a corresponding keyboard key to this command")
                elif key[3] == 'space':
                    press(Key.space)
