...
""" 
    GUI class that generates the overall user interface, buttons, dropdown menues,etc. 
    each button runs a command that is tied to a function defined outside the GUI class 
    in my version, each function that is tied to each button is practically a thread that 
    starts when the button is pressed and runs an infinite while loop
"""
...
class gui(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        threshold_label.grid(row=0, column=5, padx=(40, 0), pady=10, sticky='W')
        ...
        threshold_options.grid(row=0, column=6, padx=0, pady=10, sticky='W')

        delay_label = tk.Label(self, text="Delay time (s)", font=cooper)
        delay_label.grid(row=1, column=5, padx=(40, 0), pady=10, sticky=W)
        delay_var = IntVar(self)
        delay_var.set(0)
        delay_options = tk.OptionMenu(self, delay_var, 0, 1, 2, 3)
        delay_options.config(font=cooper)
        delay_options.grid(row=1, column=6, padx=0, pady=10, sticky='W')

        application_label = tk.Label(self, text="Choose application", font=cooper)
        application_label.grid(row=2, column=5, padx=(40, 0), pady=10, sticky='W')
        application_var = StringVar(self)
        application_var.set("BCI Paint     ")
        application_options = tk.OptionMenu(self, application_var, "BCI Paint     ", "BCI Gaming")
        application_options.config(font=cooper)
        application_options.grid(row=2, column=6, padx=0, pady=10, sticky='W')

        profile_entry = "GRH-HomeBCI"
        send_button = tk.Button(master=self, bg='green', text="Send Key", fg="white",
                                font=cooper, width=8,
        ...
        send_button.grid(row=5, column=0, pady=20, columnspan=2)

        pause_button = tk.Button(master=self, bg='tomato', text='pause', width=8, fg="white", font=cooper,

                                 command=lambda: stop_thread())
        pause_button.grid(row=5, column=2, pady=30, columnspan=2)

        Launch_button = tk.Button(master=self, bg='green', text='Launch Application', width=16, fg="white", font=cooper,

                                  command=lambda: launch_app(application_var.get()))
        Launch_button.grid(row=5, column=4, pady=30, columnspan=2)
        ...
        def headset_thread(profile, threshold, dly, key):
            global com_data
            try:
                ...
                """
                    individual data values if needed for other applicaitons 
                """
                new_data = c.ws.recv()
        ...
        """
            threads and functions corresponding to each button 
        """

        def start_thread(profile, threshold, dly, key):
            global flag
            flag.set('go')
            send_thread = threading.Thread(target=headset_thread, args=[profile, threshold, dly, key], daemon=True)
            send_thread.start()
        ...
        def launch_app(application: str):
            ...
            if application.strip() == "BCI Paint":
                subprocess.run(["start", os.path.join("dist", "main", "main.exe")], capture_output=True, shell=True)
                # BCI_paint_thread = threading.Thread(target=BCI_paint, args=[], daemon=True)
                # BCI_paint_thread.start()
